local function getSystemColour(amount)
  ...
  function nightstalker.ui.update_mindsim()
    if not gmcp.Char.Vitals then return end
  
    local class = gmcp.Char.Status.class
  
    if not nightstalker.ui.footer then nightstalker.ui.create_gauges(class) end
  
    nightstalker.ui.health:setValue(tonumber(gmcp.Char.Vitals.hp), tonumber(gmcp.Char.Vitals.maxhp), '<center><font color="LightGrey"><font color="LightGrey">Health: '..string.format('%s / %s', gmcp.Char.Vitals.hp, gmcp.Char.Vitals.maxhp))
  
    if class == 'Nanoseer' then
      nightstalker.ui.nanites:setValue(tonumber(gmcp.Char.Vitals.nn), tonumber(gmcp.Char.Vitals.maxnn), '<center><font color="LightGrey">Nanites: '..gmcp.Char.Vitals.formatnn)
      nightstalker.ui.sanity:setValue(tonumber(gmcp.Char.Vitals.sa), tonumber(gmcp.Char.Vitals.maxsa), '<center><font color="LightGrey">Sanity: '..gmcp.Char.Vitals.formatsa)
    elseif class == 'Engineer' then
      nightstalker.ui.parts:setValue(tonumber(gmcp.Char.Vitals.pt), tonumber(gmcp.Char.Vitals.maxpt), '<center><font color="LightGrey">Parts: '..gmcp.Char.Vitals.formatpt)
    elseif class == 'B.E.A.S.T.' then
      nightstalker.ui.plasma:setValue(tonumber(gmcp.Char.Vitals.pl), tonumber(gmcp.Char.Vitals.maxpl), '<center><font color="LightGrey">Plasma: '..gmcp.Char.Vitals.formatpl)
    elseif class == 'Scoundrel' then
      nightstalker.ui.bullets:setValue(tonumber(gmcp.Char.Vitals.bl), tonumber(gmcp.Char.Vitals.maxbl or 8), '<center><font color="LightGrey">Bullets: '..gmcp.Char.Vitals.formatbl)
    elseif class == 'Fury' then
      nightstalker.ui.rage:setValue(tonumber(gmcp.Char.Vitals.rg), 1000, '<center><font color="LightGrey">Rage: '..gmcp.Char.Vitals.formatrg)
    end
  ...
  nightstalker.ui.bottomleft:echo([[
  ...
  nightstalker.registerevent('mindsim vitals', 'gmcp.Char.Vitals', 'nightstalker.ui.update_mindsim')
  nightstalker.registerevent('mindsim status', 'gmcp.Char.Status', 'nightstalker.ui.update_mindsim')